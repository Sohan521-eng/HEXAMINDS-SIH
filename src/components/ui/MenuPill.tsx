...
export const MenuPill: React.FC<MenuPillProps> = ({
  ...
  useEffect(() => {
    const layout = () => {
      ...
      const white = button.querySelector(".pill-label-hover");
  ...
  return (
    <button
      ...
      type="button"
      onClick={onClick}
      className={`menu-pill ${className}`}
      onMouseEnter={handleEnter}
      onMouseLeave={handleLeave}
      aria-label="Open mission control tactical drawer"
    ...
    >
      ...
      <span className="label-stack">
        <span className="pill-label">
          <Menu className="w-4 h-4 sm:w-5 sm:h-5 text-[#00F2FE] drop-shadow-[0_2px_4px_rgba(0,0,0,0.95)]" />
        </span>
        <span className="pill-label-hover" aria-hidden="true">
          <Menu className="w-4 h-4 sm:w-5 sm:h-5 text-[#050B14]" />
        </span>
      </span>
    </button>
  );
