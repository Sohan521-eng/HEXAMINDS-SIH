...
export const SplitFlapText: React.FC<SplitFlapTextProps> = ({
  ...
  useEffect(() => {
    ...
    // If we already have a current phrase that differs from the target, animate the flip!
    const fromPhrase = currentTextRef.current
      ? normalizePhrase(currentTextRef.current, width)
      : targetPhrase;
    ...
    const animateTo = (destPhrase: string) => {
      if (prefersReducedMotion) {
        currentTextRef.current = destPhrase;
        setTiles(createTiles(destPhrase));
        return 0;
      }

      const prev = normalizePhrase(currentTextRef.current || destPhrase, width);
      const targetChars = destPhrase.split("");

      const plans = targetChars
        .map((targetChar, index) => {
          const fromChar = prev[index] || " ";
    ...
    if (fromPhrase !== targetPhrase && currentTextRef.current) {
  ...
  return (
    <div
      ...
      style={componentStyle as CSSProperties}
      role="text"
      aria-label={settledText || undefined}
      {...props}
    >
      {tiles.map((tile, index) => {
        if (tile.current === " " && !tile.flipping) {
          return (
            <span
